...
public interface MemberWalletDao extends BaseDao<MemberWallet> {
    ...
    List<MemberWallet> geteveryabc(@Param("coinName")String coinName,@Param("weekDay")int weekDay);
    ...
    @Transactional
    @Modifying
    @Query(value="UPDATE member_wallet SET balance=balance+:teamBalance where coin_id = 'abc' AND member_id=:teamId",nativeQuery = true)
    ...
    @Transactional(rollbackFor = Exception.class)
    @Modifying
    @Query(value = "UPDATE member_wallet SET balance=balance+:balance WHERE coin_id='abc' AND member_id=:memberId",nativeQuery = true)
    int increaseBalanceForabc(@Param("balance")BigDecimal mineAmount,@Param("memberId") Long memberId);

//    //初始化超级合伙人abc的数量
//    @Modifying
//    @Transactional
//    @Query(value="update member_wallet  set balance = '20' where member_id = :memberId and coin_id = 'abc' ",nativeQuery = true)
