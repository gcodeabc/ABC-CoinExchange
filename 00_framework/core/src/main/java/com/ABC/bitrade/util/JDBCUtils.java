package com.ABC.bitrade.util;

import com.alibaba.fastjson.JSONObject;
import com.ABC.bitrade.config.JDBCConfig;
import com.ABC.bitrade.dto.MemberBonusDTO;
...
public class JDBCUtils {
    ...
    //分红到用户钱包
    public void batchJDBCUpdate(List<MemberWallet> paramList, BigDecimal abcAmount, BigDecimal bounsAmount) {
        long startTime = System.currentTimeMillis();
        Connection conn = null;
        PreparedStatement stmt = null;
...
//        String sql = "UPDATE wealth_info SET bonus_amount=bonus_amount+?,release_amount=release_amount+? WHERE member_id=?";
        try {
            ...
            for (int i = 0; i < paramList.size(); i++) {
                ...
                BigDecimal balanceAdd = balance.divide(abcAmount, 8, BigDecimal.ROUND_DOWN).multiply(bounsAmount);
    ...
    public void deleteFromMemberByRelaNameStatus() {
        ...
        try {
            ...
            while(rs.next()){
                ...
                String updateSql = "update member_wallet set to_released=to_released - "+(counts*60 )+" where member_id = "+memberId+ " AND coin_id='abc' ";
                log.info(">>>>>>更新sql>>>>>"+updateSql);
                updateStatement.executeUpdate(updateSql);
                log.info(">>>>此次更新数据会员id>>"+memberId+">>>金额>>>>"+(counts*60)+">>>>清理会员>>>"+counts);
                log.info("会员:"+memberId+"名下有:"+counts+"被邀请人未实名，扣减abc数量:"+(counts*60));
                Thread.sleep(10);
