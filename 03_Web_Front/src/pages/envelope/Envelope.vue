<template>
  <div class="envelope">
    ...
    <p style="margin-top: -80px;margin-bottom: 80px;text-align:center;color: rgb(255, 136, 79);font-size:13px;">该红包通过币严(ABC.COM)发出</p>
    ...
    <router-link style="font-size:14px;text-align:center;color: #EEE;margin-top:15px;text-decoration:underline;" to="/app">© ABC.COM | 下载APP</router-link>
    </p>
...
export default {
  data() {
    return {
      spinShow: false,
      hasReceived: false,
      inviteLink: "https://www.ABC.com/envelope/",
      envelopeNo: "",
      country: "",
      ...
      hasInviteUser: false,
      inviteUserId: "***********",
      inviteUserAvatar: "https://ABC.oss-cn-hangzhou.aliyuncs.com/defaultavatar.png",
      envelopeInfo: {
        id: 0,
        name: "**********",
        totalAmount: "0.00",
        ...
        invite: 0,
        type: 0,
        logo: "https://ABC.oss-cn-hangzhou.aliyuncs.com/applogo.png",
        bgImage: "https://ABC.oss-cn-hangzhou.aliyuncs.com/redenvelope.png",
        state: 0,
        detail: "留给未来一个暴富的可能"
      },
      formInline: {
    ...
    getEnvelope(){
        ...
        this.$http.post(this.host + "/uc/redenvelope/query", param).then(res => {
          ...
          if (res.status == 200 && res.body.code == 0) {
            ...
            window.document.title = "【" + this.envelopeInfo.totalAmount + " " + this.envelopeInfo.unit + "】" + this.envelopeInfo.name + "  — 币严(ABC.COM)交易所"
            this.getEnvelopeDetailList();
          } else {
              this.$Message.error(res.body.message);
          }
